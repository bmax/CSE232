    ...
    for (int hold_value = 10; hold_value<=30;hold_value+=5) {
      if (ii ==1) {
        ...
        if (hold_value!=10) { 
          ...
          cout << "Player one won: " << wins.first << " & Player two won " << wins.second << " & Player one percentage: " <<perc<<"%"<<endl;
        }
      }
      if (ii ==2) {
        pair<long,long> wins;
        if (hold_value!=15) { 
          for (int start_games=0; start_games<num_games;start_games++) {
            int whowon = versus(player_one, 15, player_two, hold_value, eng);
            add_wins(whowon, wins);
            total_games++;
          }
      ...
      if (ii == 3) {
        ...
        if (hold_value!=20) {
          for (int start_games=0; start_games<num_games;start_games++) {
            int whowon = versus(player_one, 20, player_two, hold_value, eng);
            add_wins(whowon,wins);
            total_games++;
          }
      ...
      if (ii == 4) {
        ...
        if (hold_value!=25) { 
          for (int start_games=0; start_games<num_games;start_games++) {
            int whowon = versus(player_one, 25, player_two, hold_value, eng);
            add_wins(whowon,wins);
            total_games++;
          }
      ...
      if (ii == 5) {
        ...
        if (hold_value!=30) { 
          for (int start_games=0; start_games<num_games;start_games++) {
            int whowon = versus(player_one, 30, player_two, hold_value, eng);
            add_wins(whowon,wins);
            total_games++;
          }
